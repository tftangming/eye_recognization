...
+                (N,128,128,3) -> (N,64,64,24)
        ...
        with tf.variable_scope('Head'):
            conv1_ = self._convblock(x=X, name='conv1_3X3', shape=[3, 3, 3, 2*K], bn_istraing=bn_istraining,
                                   padding='SAME', stride=[1, 1], mode='conv_bn_relu')
    ...
    def trans_block(self, input_x, name, theta=0.5):
        '''
        连接两个Denseblock的中间环节，主要作用：1X1 CONV 缩减通道数C -> 2X2 avgpool 缩减特征图尺寸H,W
    ...
    def dense_block(self, input_x, name, growth_tate=12, L=5):
        ...
        :param bn_istraning:BN训练标志位
        :param name:
        :param growth_tate: 每个bottleneck_layer的输出是[N,H,W,K],设为12
    ...
    def _bottleneck_layer(self, input_x, growth_rate, bn_istraning, name):
        ...
        :param name: 每层名字
        :return:[N,H,W,K0]->[N,H,W,4*K]->[N,H,W,K]
        '''
        input_depth = int(input_x.get_shape()[-1])
